...
LOG_DIR: str = os.getenv("SCANNER_LOG_DIR", SCRIPT_DIR)
LOG_FILE_PATH: str = os.path.join(LOG_DIR, "scanner_operations.log")

# Lambda's filesystem is read-only outside /tmp, and CloudWatch Logs
# already captures stdout durably — there's no reason to also write a
# log file there. Locally (default), file logging stays on exactly as
# before; set SCANNER_LOG_TO_FILE=false (done in the Lambda's env vars)
# to skip the FileHandler entirely.
LOG_TO_FILE: bool = os.getenv("SCANNER_LOG_TO_FILE", "false").strip().lower() != "false"

_log_handlers: list[logging.Handler] = [logging.StreamHandler(sys.stdout)]
if LOG_TO_FILE:
    _log_handlers.append(logging.FileHandler(LOG_FILE_PATH, encoding="utf-8"))
...
STATE_DIR: str         = os.getenv("SCANNER_STATE_DIR", os.path.join(SCRIPT_DIR, "state"))
...
# ===========================================================================
#
# Maps each alert_type to a MITRE ATT&CK Enterprise technique + tactic, so
# every finding can be explained in attack-framework terms instead of just
# scanner-specific language. IDs verified against attack.mitre.org.
#
# Not every alert_type gets a technique. Off-Hours Login and Geo Anomaly
# are behavioural/contextual signals — they support detecting a Valid
# Accounts (T1078) compromise but are not themselves a catalogued ATT&CK
# technique. Forcing an ID onto them would be less accurate than admitting
# they're supporting evidence, not techniques.

@dataclass(frozen=True)
class MitreRef:
    """
    A MITRE ATT&CK Enterprise reference attached to one alert_type.

    technique / technique_name / tactic are all None when the alert_type
    has no standalone ATT&CK technique (see module note above) — callers
    should treat this as "no mapping", not an error.
    """
    technique:      str | None
...
MITRE_MAPPING: dict[str, MitreRef] = {
    # Persistence via authentication tampering
    "MFA Disabled":               MitreRef("T1556.006", "Modify Authentication Process: Multi-Factor Authentication", "Persistence"),
    "MFA Disabled (Drift)":       MitreRef("T1556.006", "Modify Authentication Process: Multi-Factor Authentication", "Persistence"),
    # Persistence via credential manipulation
    "Stale Key":                  MitreRef("T1098.001", "Account Manipulation: Additional Cloud Credentials", "Persistence"),
    "Never Used Key":              MitreRef("T1098.001", "Account Manipulation: Additional Cloud Credentials", "Persistence"),
    "Multiple Active Keys":        MitreRef("T1098.001", "Account Manipulation: Additional Cloud Credentials", "Persistence"),
    "Disabled Account With Key":   MitreRef("T1098.001", "Account Manipulation: Additional Cloud Credentials", "Persistence"),
    "Key Age Jump":                MitreRef("T1098.001", "Account Manipulation: Additional Cloud Credentials", "Persistence"),
    # Initial access / persistence via valid (cloud) accounts
    "No Password Rotation":        MitreRef("T1078.004", "Valid Accounts: Cloud Accounts", "Initial Access"),
    "Stale Account":               MitreRef("T1078.004", "Valid Accounts: Cloud Accounts", "Initial Access"),
    "Dormant Admin":               MitreRef("T1078.004", "Valid Accounts: Cloud Accounts", "Initial Access"),
    # Persistence via new account creation
    "New User Created":            MitreRef("T1136.003", "Create Account: Cloud Account", "Persistence"),
    # Privilege escalation via role/policy manipulation
    "Role Escalation":             MitreRef("T1098.003", "Account Manipulation: Additional Cloud Roles", "Privilege Escalation"),
    "Role Changed":                MitreRef("T1098.003", "Account Manipulation: Additional Cloud Roles", "Privilege Escalation"),
    "Wildcard Permission":         MitreRef("T1098.003", "Account Manipulation: Additional Cloud Roles", "Privilege Escalation"),
    # Default/root account abuse
    "Root Account Used":           MitreRef("T1078.001", "Valid Accounts: Default Accounts", "Initial Access"),
    # Credential access
    "Brute Force Indicator":       MitreRef("T1110", "Brute Force", "Credential Access"),
    # Behavioural signals — no standalone technique (see module note)
    "Off-Hours Login":             MitreRef(None, None, None),
    "Geo Anomaly":                 MitreRef(None, None, None),
}


def get_mitre_mapping(alert_type: str) -> MitreRef:
    """
    Look up the MITRE ATT&CK technique + tactic for a detector's alert_type.

    Never raises — an alert_type with no entry (including future detector
    types nobody has catalogued yet) degrades to MitreRef(None, None, None)
    rather than crashing a scan over missing threat-intel metadata.

    Args:
        alert_type: Key from RISK_BASE_SCORES (e.g. "Wildcard Permission").

    Returns:
        The matching MitreRef, or an empty one if there is no mapping.
    """
    return MITRE_MAPPING.get(alert_type, MitreRef(None, None, None))
...
class Finding:
    ...
    mitre_technique : MITRE ATT&CK technique ID (e.g. "T1098.001"), or None
                      if this alert_type is a behavioural signal rather
                      than a standalone technique (see MITRE_MAPPING).
    mitre_tactic    : MITRE ATT&CK tactic name (e.g. "Persistence"), or None.
    """
    username:        str
    alert_type:      str
    severity:        Severity
    ...
    mitre_technique: str | None = field(init=False)
    mitre_tactic:    str | None = field(init=False)

    def __post_init__(self) -> None:
        self.sla         = SEVERITY_SLA[self.severity]
        self.fingerprint = _make_fingerprint(self.username, self.alert_type)
        mitre                 = get_mitre_mapping(self.alert_type)
        self.mitre_technique  = mitre.technique
        self.mitre_tactic     = mitre.tactic
...
def _build_safe_prompt(high_risk_findings: list[Finding]) -> str:
    ...
    before interpolation to mitigate prompt injection. Each finding's
    MITRE ATT&CK technique/tactic (when mapped) is included so the model
    grounds its narrative in the attack framework instead of inventing
    its own threat language.
    ...
    safe_findings: list[dict[str, Any]] = [
        {
            ...
            "mitre_technique":  f.mitre_technique,       # e.g. "T1098.001" or None
            "mitre_tactic":     f.mitre_tactic,           # e.g. "Persistence" or None
        }
    ...
    return (
        ...
        "Group findings by severity. Where a finding includes a non-null "
        "mitre_technique, reference that MITRE ATT&CK technique by ID and name "
        "when explaining it. Recommend specific remediation steps. "
...
def _select_relevant_cloudtrail_events(
    finding: Finding,
    cloudtrail_events: list[dict[str, Any]],
    max_events: int = 15,
) -> list[dict[str, Any]]:
    """Prioritize CloudTrail events relevant to the finding."""
    priorities={
        "Root Account Used":{"ConsoleLogin","AssumeRole","CreateAccessKey","DeleteAccessKey","PutUserPolicy","AttachUserPolicy"},
        "Wildcard Permission":{"PutUserPolicy","AttachUserPolicy","CreatePolicy","CreatePolicyVersion"},
        "Brute Force Indicator":{"ConsoleLogin"},
    }
    wanted=priorities.get(finding.alert_type,set())
    if not wanted:
        return cloudtrail_events[:max_events]
    ranked=[e for e in cloudtrail_events if e.get("event_name") in wanted]
...
def _build_incident_prompt(finding: Finding, events: list[dict[str, Any]]) -> str:
    lines=[]
    if not events:
        lines.append("- No relevant CloudTrail events were found.")
    for e in events:
        lines.append(
            f"- Time: {sanitise_for_prompt(str(e.get('event_time','Unknown')))} | "
            f"Event: {sanitise_for_prompt(str(e.get('event_name','Unknown')))} | "
            f"Source IP: {sanitise_for_prompt(str(e.get('source_ip','Unknown')))} | MFA: "
            f"{'Yes' if e.get('mfa_used') else 'No' if e.get('mfa_used') is not None else 'Unknown'} | "
            f"Region: {sanitise_for_prompt(str(e.get('aws_region','Unknown')))} | "
            f"Service: {sanitise_for_prompt(str(e.get('event_source','Unknown')))} | "
            f"UserAgent: {sanitise_for_prompt(str(e.get('user_agent','Unknown')))}"
        )
    return f"""
You are a Senior AWS Cloud Security Incident Responder.

Analyse ONLY the evidence provided.
Never invent AWS services, APIs or attack techniques.
If evidence is insufficient, explicitly state that.

Finding
-------
Type: {sanitise_for_prompt(finding.alert_type)}
Severity: {finding.severity.value}
User: {sanitise_for_prompt(finding.username)}
Scanner Detail: {sanitise_for_prompt(finding.detail)}
...
def generate_incident_explanation(
    ...
    events=_select_relevant_cloudtrail_events(finding, cloudtrail_events)
    prompt=_build_incident_prompt(finding, events)
    endpoint=f"{OLLAMA_BASE_URL}/api/generate"
    try:
        response=requests.post(endpoint,json={"model":OLLAMA_MODEL,"prompt":prompt,"stream":False},timeout=OLLAMA_TIMEOUT_SEC)
        response.raise_for_status()
        ai_text=response.json().get("response","").strip()
        if not ai_text:
            logger.warning("AI engine returned an empty explanation.")
            return None
        logger.info("Incident explanation generated for %s (%d chars).",finding.username,len(ai_text))
        return ai_text
    except requests.exceptions.ConnectionError:
        logger.error("Cannot reach Ollama at %s — run: ollama serve", endpoint)
    except requests.exceptions.Timeout:
